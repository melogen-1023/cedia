...
class BiLiBiLi(source.Source):
    ...
    def video(self):
        ...
        for child in soup.body.children:
            ...
            if temp_str == 'window.__playinfo__':
                tag = child
                kind = 'window.__playinfo__'
                break
            if temp_str == 'window.__INITIAL_STATE__':
                tag = child
                kind = 'window.__INITIAL_STATE__'
                break
        if tag is None:
            task_logger.info(f'无法找到视频信息,[无__playinfo__,无__INITIAL_STATE__]')
            return -1

        if kind == 'window.__playinfo__':
            self.play_info_handle(tag)
        if kind == 'window.__INITIAL_STATE__':
            self.initial_state_handle(tag)
    ...
    def play_info_handle(self, play_info_tag):
        copy_index_l = 0
        ...
        self.media_info['duration'] = int(duration_s / 60)
    ...
    def initial_state_handle(self, initial_state_tag):
        stack = []
        json_str = ''
        start = False
        result = True
        for c in initial_state_tag.string:
            if c == '{':
                start = True
                stack.append(c)
            if c == '}':
        ...
        if result:
            ...
            ep = loaded_json['epList'][0]
            response = requests.get("https://api.bilibili.com/pgc/player/web/playurl", params={
                "ep_id": ep['id'],
                "bvid": ep['bvid'],
                "cid": ep['cid'],
                'qn': '0',
